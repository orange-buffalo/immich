...
void main() {
  ...
  group('RestoreAction', () {
    ...
    testWidgets('restores trashed assets of a partner who granted delete permission', (tester) async {
      final mine = owned();
      final partner = RemoteAssetFactory.create(deletedAt: .new(2020));
      context.partnerDeleteGrants = {partner.ownerId};

      await pumpRestore(tester, {mine, partner});

      verify(() => assetService.restoreTrash([mine.id, partner.id])).called(1);
    });
