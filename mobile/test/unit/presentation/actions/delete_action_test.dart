...
void main() {
  ...
  group('DeleteAction', () {
    group('trash', () {
      ...
      // fork-only: partner delete permission, see FORK.md
      testWidgets('trashes assets of a partner who granted delete permission', (tester) async {
        final mine = owned();
        final partner = RemoteAssetFactory.create();
        context.partnerDeleteGrants = {partner.ownerId};

        await pumpDelete(tester, {mine, partner});
        await tester.pumpAndSettle();

        verify(() => assetService.trash([mine.id, partner.id])).called(1);
      });

      testWidgets('never permanently deletes a partner asset', (tester) async {
        final mine = owned();
        final partner = RemoteAssetFactory.create();
        context.partnerDeleteGrants = {partner.ownerId};

        await pumpDelete(tester, {mine, partner}, trashEnabled: false);
        await respondToDialog(tester, confirm: true);

        verify(() => assetService.delete([mine.id])).called(1);
        verifyNever(() => assetService.trash(any()));
      });

      testWidgets('is hidden for partner assets when the trash feature is disabled', (tester) async {
        final partner = RemoteAssetFactory.create();
        context.partnerDeleteGrants = {partner.ownerId};
        when(
          () => context.service.serverInfo.getServerFeatures(),
        ...
        await tester.pumpTestWidget(
          context,
          const ActionIconButton(action: DeleteAction(source: .timeline)),
          overrides: context.selected({partner}),
        );
        final scope = ProviderScope.containerOf(tester.element(find.byType(ActionIconButton)), listen: false);
        await scope.read(serverInfoProvider.notifier).getServerFeatures();
        await tester.pumpAndSettle();
