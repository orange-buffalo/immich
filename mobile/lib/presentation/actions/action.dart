...
/// Fork-only, see FORK.md. Own assets plus those of partners who granted the current user delete permission.
final deletableAssetsActionProvider = Provider.family.autoDispose<AssetFilter<RemoteAsset>, ActionSource>((
  ref,
  source,
) {
  final ownerIds = ref.watch(deletableOwnerIdsProvider);
  return ref.watch(assetsActionProvider(source)).remote().where((asset) => ownerIds.contains(asset.ownerId));
}, dependencies: [assetsActionProvider]);
