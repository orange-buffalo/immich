...
final _stateProvider = Provider.family.autoDispose<_State?, ActionSource>((ref, source) {
  ...
  // fork-only: partners who granted delete permission can have their assets trashed, see FORK.md
  final deletableOwnerIds = ref.watch(deletableOwnerIdsProvider);

  final localIds = <String>[];
  final ownedRemote = <RemoteAsset>[];
  final deletableRemote = <RemoteAsset>[];
  for (final asset in assets) {
    ...
    if (asset case final RemoteAsset remote when deletableOwnerIds.contains(remote.ownerId)) {
      deletableRemote.add(remote);
    }
  }
  ...
  final canTrash = trashEnabled && !deletableRemote.every((asset) => asset.isTrashed || asset.isLocked);
  // Permanently deleting bypasses the owner's trash, so a partner's assets are only ever trashed.
  final remote = canTrash ? deletableRemote : ownedRemote;

  if (localIds.isEmpty && remote.isEmpty) {
    return null;
  }

  final trash = remote.isEmpty || canTrash;

  return (localIds: localIds, remoteIds: remote.map((asset) => asset.id).toList(growable: false), trash: trash);
